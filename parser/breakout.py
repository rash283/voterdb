...
residence_address = residence_address.assign(full_address=lambda x: (x['ResidenceStreet'] + ', ' +
        x['ResidenceCity'] + ', ' + x['ResidenceState'] + ', ' + x['ResidenceZip']))
mailing_address.to_csv('mailing_addresses.csv')
residence_address.to_csv('residence_addresses.csv')
mailing_address.loc[:, 'MailingStreet'].to_csv('mailing_streets.csv', index=False)
df.to_csv()
pattern = r'^(?P<house_num>\d+?(?: 1/2)?) (?P<street>.+?)$'
